...
function diag(stage,extra={}){try{window.__ocrDiagnostic?.({time:new Date().toISOString(),stage,detail:'long-code-v4',...extra})}catch(_){} }
...
function matchBox(data,target,ox,oy){
  ...
  for(const l of data?.lines||[]){
    ...
    const k=codeKey(l.text),score=sim(k,wanted),contains=k.includes(wanted);
    if(contains||score>=.86)candidates.push({text:l.text,bbox:[l.bbox.x0,l.bbox.y0,l.bbox.x1,l.bbox.y1],confidence:Number(l.confidence||0),score,exact:contains||k===wanted,key:k});
  }
  ...
  for(let i=0;i<words.length;i++){
    ...
    for(let j=i;j<Math.min(words.length,i+32);j++){
      ...
      if(contains||near){candidates.push({text,bbox,confidence,score,exact:contains||k===wanted,key:k});break}
...
async function findOnPage(page,target){const pix=page.toPixmap(mupdf.Matrix.scale(SCALE,SCALE),mupdf.ColorSpace.DeviceRGB,false,false),bitmap=await createImageBitmap(new Blob([pix.asPNG()],{type:'image/png'}));try{const rw=Math.max(1,Math.ceil(bitmap.width*RIGHT_FRACTION)),rx=bitmap.width-rw,bh=Math.max(1,Math.ceil(bitmap.height*BOTTOM_FRACTION)),by=bitmap.height-bh;const bottom=canvasCrop(bitmap,rx,by,rw,bh);let data=await recognize(bottom,6),m=matchBox(data,target,rx,by);if(m.length)return m;const strip=canvasCrop(bitmap,rx,0,rw,bitmap.height);data=await recognize(strip,6);return matchBox(data,target,rx,0)}finally{bitmap.close?.()}}
...
async function supplement(token){if(document.querySelector(OCR)?.checked!==true||token!==runToken)return;const batch=window.__batchAnalysis;if(!Array.isArray(batch)||!batch.length)return;let total=0;for(let ai=0;ai<batch.length;ai++){const item=batch[ai];if(token!==runToken||item?.error||!item?.data)continue;const pending=(item.counts||[]).filter(c=>c?.find?.trim()&&isLongDrawingCode(c.find)&&Number(c.count||0)===0&&Number(c.annotationCount||0)===0&&Number(c.ocrCount||0)===0);if(!pending.length)continue;const doc=mupdf.PDFDocument.openDocument(item.data,'application/pdf');try{for(let pi=0;pi<doc.countPages()&&pending.some(c=>Number(c.ocrCount||0)===0);pi++){if(token!==runToken)return;const s=document.querySelector(STATUS);if(s)s.textContent=`OCR código largo · ${item.name} · página ${pi+1}`;const page=doc.loadPage(pi);for(const c of pending){if(Number(c.ocrCount||0)>0)continue;let matches=[];try{matches=await findOnPage(page,c.find)}catch(e){diag('ocr.longcode.error',{file:item.name,page:pi+1,target:c.find,error:e?.message||String(e)});console.warn('long titleblock OCR v4',item.name,pi+1,e)}if(!matches.length)continue;c.ocrCount=(c.ocrCount||0)+matches.length;c.ocrMatches=(c.ocrMatches||[]).concat(matches.map(m=>({...m,page:pi+1})));c.pages=c.pages||[];if(!c.pages.includes(pi+1))c.pages.push(pi+1);total+=matches.length;refreshRow(ai)}}}finally{doc.destroy()}}if(total){const stat=document.querySelector('#statEdits');if(stat)stat.textContent=batch.reduce((sum,a)=>sum+(a?.error?0:(a.counts||[]).reduce((q,c)=>q+Number(c.count||0)+Number(c.annotationCount||0)+Number(c.ocrCount||0),0)),0);const apply=document.querySelector('#batchApply');if(apply)apply.disabled=false;const s=document.querySelector(STATUS);if(s)s.textContent=`Reconocimiento terminado: ${total} código${total===1?'':'s'} largo${total===1?'':'s'} detectado${total===1?'':'s'} en cartela.`}window.__longTitleBlockOCR={total,version:4}}

function waitForNewAnalysis(token,previous){let ticks=0;const timer=setInterval(()=>{if(token!==runToken){clearInterval(timer);return}const current=window.__batchAnalysis,btn=document.querySelector(ANALYZE);if(current!==previous&&Array.isArray(current)&&current.length&&btn&&!btn.disabled){clearInterval(timer);setTimeout(()=>supplement(token).catch(e=>console.warn('long titleblock OCR v4',e)),150);return}if(++ticks>2400)clearInterval(timer)},200)}

document.querySelector(ANALYZE)?.addEventListener('click',()=>{if(document.querySelector(OCR)?.checked!==true)return;const previous=window.__batchAnalysis;runToken++;diag('ocr.longcode.start',{version:4});waitForNewAnalysis(runToken,previous)},true);
window.__longTitleBlockOCRLoaded={version:4};
