// Completion observer for Analyze PDFs, extended with the additive cloud-family stage.
...
function publish(state,extra={}){window.__analysisCompletionState={version:6,cycle:state.id,startedAt:state.startedAt,running:!state.finished,complete:false,...extra};window.dispatchEvent(new CustomEvent('analysis-completion-state',{detail:window.__analysisCompletionState}));}
...
function finalize(state){if(state.finished)return;state.finished=true;if(timer){clearInterval(timer);timer=null;}const itemErrors=(Array.isArray(window.__batchAnalysis)?window.__batchAnalysis:[]).filter(x=>x?.error).length,warnings=itemErrors+cloudErrorsAfter(state.startedAt).length+Number(state.annotationWarnings||0);window.__analysisCompletionState={version:6,cycle:state.id,startedAt:state.startedAt,finishedAt:now(),running:false,complete:warnings===0,warnings,stages:{base:true,ocr:true,horizontalLongCode:!state.longCodeRequired||state.horizontalDone,verticalLongCode:!state.longCodeRequired||state.verticalDone,legacyVertical:!state.longCodeRequired||state.legacyDone,annotation:true,vectorCloud:!state.cloudSelected||state.vectorDone,zeroPending:!state.cloudSelected||state.zeroDone,colorOptional:!state.cloudSelected||state.colorDone,redCurvedCycle:!state.cloudSelected||state.redCycleDone,additiveCloudFamilies:!state.cloudSelected||state.additiveDone}};window.dispatchEvent(new CustomEvent('analysis-completion-state',{detail:window.__analysisCompletionState}));}
function beginCycle(){if(!hasSelectedFiles())return;const id=++cycle;if(timer){clearInterval(timer);timer=null;}const state={id,startedAt:now(),previousBatch:window.__batchAnalysis,cloudSelected:selectedClouds(),annotationRequired:needsAnnotationMetadata(),annotationStarted:false,annotationDone:!needsAnnotationMetadata(),annotationWarnings:0,vectorBaseline:window.__revisionCloudVectorFallbackDebug,zeroBaseline:Number(window.__revisionCloudZeroPendingVersion||0),colorBaseline:window.__revisionCloudColoredOptionalState,redCycleBaseline:Number(window.__revisionCloudRedCycleVersion||0),additiveBaseline:Number(window.__revisionCloudAdditiveFamilyVersion||0),longBaseline:window.__longTitleBlockOCR,verticalBaseline:window.__longTitleBlockVerticalOCR,legacyBaseline:window.__longTitleBlockVerticalLegacyOCR,lastOcrDiag:String(q(OCR_DIAG)?.textContent||''),lastOcrChange:now(),baseDone:false,baseDoneAt:0,longCodeRequired:false,ocrQuiet:false,ocrDone:false,horizontalDone:false,verticalDone:false,legacyDone:false,vectorDone:false,zeroDone:false,colorDone:false,redCycleDone:false,additiveDone:false,settledSince:0,finished:false};publish(state,{running:true,stage:'iniciando análisis'});timer=setInterval(()=>{if(id!==cycle||state.finished)return;const batch=Array.isArray(window.__batchAnalysis)?window.__batchAnalysis:[],baseShape=batch.length>0&&batch.every(x=>x?.error||x?.data),batchChanged=batch!==state.previousBatch,analyzeIdle=q(ANALYZE)?.disabled===false;if(!state.baseDone&&batchChanged&&baseShape&&analyzeIdle){state.baseDone=true;state.baseDoneAt=now();startAnnotationStage(state,batch);}updateOcr(state,batch);if(state.baseDone)updateCloudStages(state);const done=state.baseDone&&state.ocrDone&&state.annotationDone&&state.vectorDone&&state.zeroDone&&state.colorDone&&state.redCycleDone&&state.additiveDone;if(!done){state.settledSince=0;publish(state,{running:true,stage:label(state)});return;}if(!state.settledSince)state.settledSince=now();if(now()-state.settledSince<900){publish(state,{running:true,stage:'consolidando resultados'});return;}finalize(state);},120);}
function wire(){q(ANALYZE)?.addEventListener('click',beginCycle,true);}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',wire);else wire();
window.__analysisCompletionCoordinatorV2={beginCycle,version:6};
