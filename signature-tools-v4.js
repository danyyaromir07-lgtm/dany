...
async function prepare(data, rules, removeComments, removeSignatures, removeLinks, context = {}) {
  ...
  const loadKey = `annotations-load::${context.index ?? ''}::${file}`;
  perf({ action: 'start', stage: 'cargar PDF para anotaciones', key: loadKey, file, index: context.index, total: context.total, sizeBytes: byteLength(data) });
  await yieldUI();
  let doc;
  try {
    doc = await PDFDocument.load(data, { updateMetadata: false, ignoreEncryption: false });
    perf({ action: 'end', stage: 'cargar PDF para anotaciones', key: loadKey, file, index: context.index, total: context.total, sizeBytes: byteLength(data) });
  } catch (error) {
    perf({ action: 'end', stage: 'cargar PDF para anotaciones', key: loadKey, file, index: context.index, total: context.total, sizeBytes: byteLength(data), warning: error?.message || String(error) });
    throw error;
  }
  let changed = false;
  ...
  try {
    ...
    perf({ action: 'end', stage: 'guardar PDF tras anotaciones', key: saveKey, file, index: context.index, total: context.total, sizeBytes: byteLength(data), outputBytes: byteLength(output) });
    return output;
  } catch (error) {
    perf({ action: 'end', stage: 'guardar PDF tras anotaciones', key: saveKey, file, index: context.index, total: context.total, sizeBytes: byteLength(data), warning: error?.message || String(error) });
    throw error;
  }
}
function injectOptions() { const commentsBox = q(COMMENTS); const host = commentsBox?.closest('.option-box'); if (!host) return; if (!q(SIG)) { const box = document.createElement('div'); box.className = 'option-box'; box.style.marginTop = '10px'; box.innerHTML = '<label><input id="batchRemoveSignatures" type="checkbox"><span>✍️ Eliminar firmas digitales</span></label><small>Solo se eliminan al activar esta opción. La firma dejará de ser válida al modificar el PDF.</small>'; host.parentElement?.insertBefore(box, host.nextElementSibling); } if (!q(LINKS)) { const box = document.createElement('div'); box.className = 'option-box'; box.style.marginTop = '10px'; box.innerHTML = '<label><input id="batchRemoveLinks" type="checkbox"><span>🔗 Eliminar enlaces del PDF</span></label><small>Los enlaces se conservan por defecto.</small>'; host.parentElement?.appendChild(box); } }
async function afterAnalysis() { const batch = window.__batchAnalysis || []; if (!batch.length) return; for (const a of batch) { if (a?.error || !a.data) continue; try { const doc = await PDFDocument.load(a.data, { updateMetadata: false, ignoreEncryption: false }); let annotationCount = 0, signatureCount = 0, linkCount = 0; for (const page of doc.getPages()) { const annots = annotationArray(page); if (!annots) continue; for (let i = 0; i < annots.size(); i++) { const ref = annots.get(i), st = subtype(doc, ref); if (st === 'Widget') { if (isSignature(doc, ref)) signatureCount++; } else if (st === 'Link') linkCount++; else annotationCount++; } } a.annotationCount = annotationCount; a.signatureCount = signatureCount; a.linkCount = linkCount; } catch (e) { a.signatureError = e?.message || String(e); } } const summary = q(SUMMARY); if (!summary) return; let text = summary.textContent || ''; text = text.replace(/ · 📎[^·]*/g, '').replace(/ · ✍️[^·]*/g, '').replace(/ · 🔗[^·]*/g, ''); let ann = 0, sig = 0, links = 0; for (const a of batch) { ann += Number(a.annotationCount || 0); sig += Number(a.signatureCount || 0); links += Number(a.linkCount || 0); } if (ann) text += ` · 📎 ${ann} anotación${ann === 1 ? '' : 'es'} detectada${ann === 1 ? '' : 's'}`; if (sig) text += ` · ✍️ ${sig} firma${sig === 1 ? '' : 's'} detectada${sig === 1 ? '' : 's'}`; if (links) text += ` · 🔗 ${links} enlace${links === 1 ? '' : 's'} detectado${links === 1 ? '' : 's'}`; summary.textContent = text; summary.classList.remove('hidden'); }
async function prepareForApply() {
  ...
  for (let i = 0; i < batch.length; i++) {
    ...
    try {
      ...
      perf({ action: 'end', stage: 'preparar anotaciones del PDF', key, file: a.name, index: i + 1, total: batch.length, sizeBytes: byteLength(before), outputBytes: byteLength(a.data), warning: a.data === before ? 'sin cambios: buffer original reutilizado' : '' });
    } catch (error) {
      perf({ action: 'end', stage: 'preparar anotaciones del PDF', key, file: a.name, index: i + 1, total: batch.length, sizeBytes: byteLength(a.data), warning: error?.message || String(error) });
      throw error;
    }
    await yieldUI();
  }
