import './revision-clouds-v1.js?v=20260813-cloud2';
...
function resolve(doc, obj) {
  try {
    return doc.context.lookup(obj) || obj;
  } catch (_) {
    return obj;
  }
}
...
function sameRef(a, b) {
  return String(a?.objectNumber ?? '') === String(b?.objectNumber ?? '') && String(a?.generationNumber ?? '') === String(b?.generationNumber ?? '');
}
...
function annotationContents(doc, ref) {
  const obj = resolve(doc, ref);
  const c = obj?.get?.(PDFName.of('Contents'));
  try {
    return c?.decodeText?.() || '';
  } catch (_) {
...
function preservedFreeText(doc, ref, rules) {
  if (subtype(doc, ref) !== 'FreeText') return false;
  const text = annotationContents(doc, ref).replace(/\s+/g, ' ').trim().toLowerCase();
  return rules.some((r) => {
    const needle = String(r.find || '').replace(/\s+/g, ' ').trim().toLowerCase();
...
async function prepare(data, rules, removeComments, removeSignatures, removeLinks) {
  ...
  for (const page of doc.getPages()) {
    ...
    for (let i = 0; i < annots.size(); i++) {
      ...
      if (st === 'Widget') {
        if (removeSignatures && isSignature(doc, ref)) {
      ...
      if (st === 'Link') {
        if (removeLinks) changed = true;
        else keep.push(ref);
        continue;
      }

      if (removeComments && REMOVABLE.has(st) && !preservedFreeText(doc, ref, rules)) {
        changed = true;
      } else {
        keep.push(ref);
      }
...
function injectOptions() {
  const commentsBox = q(COMMENTS);
  const host = commentsBox?.closest('.option-box');
  if (!host) return;
  ...
  if (!q(SIG)) {
    ...
    box.innerHTML = '<label><input id="batchRemoveSignatures" type="checkbox"><span>✍️ Eliminar firmas digitales</span></label><small>Solo se eliminan al activar esta opción. La firma dejará de ser válida al modificar el PDF.</small>';
  ...
  if (!q(LINKS)) {
    ...
    box.className = 'option-box';
    box.style.marginTop = '10px';
    box.innerHTML = '<label><input id="batchRemoveLinks" type="checkbox"><span>🔗 Eliminar enlaces del PDF</span></label><small>Los enlaces se conservan por defecto.</small>';
...
function wire() {
  ...
  q(ANALYZE)?.addEventListener('click', () => {
    afterAnalysis().catch((e) => console.error('[annotations]', e));
  });
}
