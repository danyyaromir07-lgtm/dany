import * as mupdf from 'https://cdn.jsdelivr.net/npm/mupdf@1.28.0/dist/mupdf.js';

const ANALYZE = '#batchAnalyze';
const OCR = '#batchEnableOCR';
const STATUS = '#batchStatus';
const SCALE = 1.75;
const RIGHT_FRACTION = .12;
let workerPromise = null;
let runToken = 0;

const norm = s => String(s || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[‐‑‒–—−]/g, '-').replace(/\s+/g, ' ').trim().toLowerCase();
const canonical = s => norm(s).replace(/\s*[-]\s*/g, '-').replace(/\s*([:/_.])\s*/g, '$1');
const key = s => canonical(s).replace(/[^a-z0-9]/g, '').replace(/o/g, '0');
function lev(a, b) {
  const p = Array.from({ length: b.length + 1 }, (_, i) => i);
  for (let i = 1; i <= a.length; i++) {
    const q = [i];
    for (let j = 1; j <= b.length; j++) q[j] = Math.min(q[j - 1] + 1, p[j] + 1, p[j - 1] + (a[i - 1] === b[j - 1] ? 0 : 1));
    for (let j = 0; j < q.length; j++) p[j] = q[j];
  }
  return p[b.length];
}
const sim = (a, b) => a && b ? 1 - lev(a, b) / Math.max(a.length, b.length) : 0;
function simpleTarget(s) {
  return !String(s || '').includes('_') && key(s).length >= 4 && key(s).length <= 14 && String(s).split(/[_\-\s./:]+/).filter(Boolean).length <= 3;
}
function preservesLeadingAbbreviation(target, candidate) {
  const m = String(target || '').trim().match(/^([A-Za-z0-9])\s*\./);
  if (!m) return true;
  return new RegExp(`^${m[1]}\\s*\\.`, 'i').test(String(candidate || '').trim());
}
function area(b) { return Math.max(0, b[2] - b[0]) * Math.max(0, b[3] - b[1]); }
function overlapRatio(a, b) {
  const x = Math.max(0, Math.min(a[2], b[2]) - Math.max(a[0], b[0]));
  const y = Math.max(0, Math.min(a[3], b[3]) - Math.max(a[1], b[1]));
  return x * y / Math.max(1, area(a));
}
function iou(a, b) {
  const x = Math.max(0, Math.min(a[2], b[2]) - Math.max(a[0], b[0]));
  const y = Math.max(0, Math.min(a[3], b[3]) - Math.max(a[1], b[1]));
  const inter = x * y;
  return inter / Math.max(1, area(a) + area(b) - inter);
}
function diag(stage, extra = {}) {
  try { window.__ocrDiagnostic?.({ time: new Date().toISOString(), stage, detail: 'titleblock-primary-safe-v1', ...extra }); } catch (_) {}
}
function structuredBoxes(page) {
  try {
    const data = JSON.parse(page.toStructuredText('preserve-spans').asJSON());
    const out = [];
    for (const block of data?.blocks || []) {
      if (block?.type !== 'text') continue;
      for (const line of block?.lines || []) {
        const b = line?.bbox;
        if (Number(line?.wmode || 0) !== 0 || !b || ![b.x, b.y, b.w, b.h].every(Number.isFinite)) continue;
        out.push([b.x, b.y, b.x + b.w, b.y + b.h]);
      }
    }
    return out;
  } catch (_) { return []; }
}
const overlapsPdfText = (boxes, bbox) => boxes.some(box => overlapRatio(bbox, box) >= .18);

async function getWorker() {
  if (workerPromise) return workerPromise;
  workerPromise = import('https://esm.sh/tesseract.js@5.1.0')
    .then(({ createWorker }) => createWorker('spa+eng'))
    .then(async w => { try { await w.setParameters({ tessedit_pageseg_mode: '6', preserve_interword_spaces: '1' }); } catch (_) {} return w; })
    .catch(e => { workerPromise = null; throw e; });
  return workerPromise;
}
function indexedKey(text) {
  const raw = canonical(text).replace(/o/g, '0'), positions = [];
  let k = '';
  for (let i = 0; i < raw.length; i++) if (/[a-z0-9]/.test(raw[i])) { positions.push(i); k += raw[i]; }
  return { raw, key: k, positions };
}
function bestPartial(container, wanted) {
  const direct = container.indexOf(wanted);
  if (direct >= 0) return { start: direct, len: wanted.length, score: 1, exact: true };
  let best = null;
  for (let len = Math.max(4, wanted.length - 1); len <= Math.min(container.length, wanted.length + 1); len++) {
    for (let start = 0; start + len <= container.length; start++) {
      const score = sim(container.slice(start, start + len), wanted);
      if (!best || score > best.score) best = { start, len, score, exact: false };
    }
  }
  return best;
}
function partialWordMatch(word, target, ox) {
  const wanted = key(target), idx = indexedKey(word?.text || ''), confidence = Number(word?.confidence || 0);
  if (wanted.length < 8 || idx.key.length < wanted.length + 3 || !word?.bbox) return null;
  const p = bestPartial(idx.key, wanted);
  if (!p || (p.exact ? confidence < 10 : (confidence < 15 || p.score < .88)) || !preservesLeadingAbbreviation(target, word.text)) return null;
  const startRaw = idx.positions[p.start], endPos = idx.positions[Math.min(idx.positions.length - 1, p.start + p.len - 1)];
  if (startRaw == null || endPos == null) return null;
  const endRaw = endPos + 1, total = Math.max(1, idx.raw.length), b = word.bbox, width = Math.max(1, b.x1 - b.x0);
  const x0 = b.x0 + width * (startRaw / total), x1 = b.x0 + width * (endRaw / total);
  return { bbox: [(x0 + ox) / SCALE, b.y0 / SCALE, (x1 + ox) / SCALE, b.y1 / SCALE], confidence, similarity: p.score, ocrText: word.text, exact: p.exact, titleBlockFallback: true, partialWithinOCR: true, containerText: word.text };
}
function findWords(data, target, ox) {
  const wanted = key(target), words = (data?.words || []).filter(w => w?.text?.trim() && w.bbox), found = [];
  for (let i = 0; i < words.length; i++) {
    const partial = partialWordMatch(words[i], target, ox);
    if (partial) { found.push(partial); continue; }
    let text = '', box = null, confidence = 100;
    for (let j = i; j < Math.min(words.length, i + 12); j++) {
      const w = words[j];
      text = text ? `${text} ${w.text}` : w.text;
      box = box ? [Math.min(box[0], w.bbox.x0), Math.min(box[1], w.bbox.y0), Math.max(box[2], w.bbox.x1), Math.max(box[3], w.bbox.y1)] : [w.bbox.x0, w.bbox.y0, w.bbox.x1, w.bbox.y1];
      confidence = Math.min(confidence, Number(w.confidence || 0));
      const k = key(text), score = sim(k, wanted), exact = k === wanted;
      if ((exact || (confidence >= 55 && score >= .84)) && preservesLeadingAbbreviation(target, text)) {
        found.push({ bbox: [(box[0] + ox) / SCALE, box[1] / SCALE, (box[2] + ox) / SCALE, box[3] / SCALE], confidence, similarity: score, ocrText: text, exact, titleBlockFallback: true });
        break;
      }
      if (k.length > wanted.length + 20) break;
    }
  }
  const unique = [];
  for (const m of found) {
    const same = unique.find(u => iou(u.bbox, m.bbox) >= .6);
    if (same) { if (m.confidence > same.confidence) Object.assign(same, m); }
    else unique.push(m);
  }
  return unique;
}
async function recognizeRightStrip(page) {
  const pix = page.toPixmap(mupdf.Matrix.scale(SCALE, SCALE), mupdf.ColorSpace.DeviceRGB, false, false);
  const bitmap = await createImageBitmap(new Blob([pix.asPNG()], { type: 'image/png' }));
  try {
    const sw = Math.max(1, Math.ceil(bitmap.width * RIGHT_FRACTION)), sx = bitmap.width - sw, canvas = document.createElement('canvas');
    canvas.width = sw; canvas.height = bitmap.height; canvas.getContext('2d').drawImage(bitmap, sx, 0, sw, bitmap.height, 0, 0, sw, bitmap.height);
    const worker = await getWorker();
    try { await worker.setParameters({ tessedit_pageseg_mode: '6', preserve_interword_spaces: '1' }); } catch (_) {}
    return { data: (await worker.recognize(canvas))?.data || null, ox: sx };
  } finally { bitmap.close?.(); try { pix.destroy?.(); } catch (_) {} }
}
function updateUI(batch, total) {
  if (!total) return;
  const rows = [...document.querySelectorAll('.batch-result')];
  batch.forEach((a, i) => {
    const span = rows[i]?.querySelector(':scope > span'); if (!span) return;
    const hits = (a.counts || []).filter(c => c.ocrMatches?.some(m => m.titleBlockFallback && !m.safeTitleblockCode)).map(c => `${c.ocrMatches.filter(m => m.titleBlockFallback && !m.safeTitleblockCode).length}× ${c.find} (vector/OCR cartela)`);
    if (hits.length && !span.textContent.includes('vector/OCR cartela')) span.append(document.createTextNode(' · ' + hits.join(' · ')));
  });
  const stat = document.querySelector('#statEdits');
  if (stat) stat.textContent = batch.reduce((sum, a) => sum + (a?.error ? 0 : (a.counts || []).reduce((q, c) => q + Number(c.count || 0) + Number(c.annotationCount || 0) + Number(c.ocrCount || 0), 0)), 0);
  const status = document.querySelector(STATUS);
  if (status) status.textContent = `Reconocimiento terminado: ${total} coincidencia${total === 1 ? '' : 's'} adicional${total === 1 ? '' : 'es'} detectada${total === 1 ? '' : 's'} en cartela vectorial. No se ha modificado ningún PDF.`;
}
async function supplement(token) {
  if (document.querySelector(OCR)?.checked !== true || token !== runToken) return;
  const batch = Array.isArray(window.__batchAnalysis) ? window.__batchAnalysis : [];
  if (!batch.length) return;
  let total = 0;
  for (const item of batch) {
    if (token !== runToken || item?.error || !item?.data || item?.kinds?.vector !== true) continue;
    const pending = (item.counts || []).filter(c => c?.find?.trim() && simpleTarget(c.find) && Number(c.count || 0) === 0 && Number(c.annotationCount || 0) === 0 && Number(c.ocrCount || 0) === 0);
    if (!pending.length) continue;
    const doc = mupdf.PDFDocument.openDocument(item.data, 'application/pdf');
    try {
      for (let pi = 0; pi < doc.countPages() && pending.some(c => Number(c.ocrCount || 0) === 0); pi++) {
        if (token !== runToken) return;
        const status = document.querySelector(STATUS); if (status) status.textContent = `OCR cartela · ${item.name} · página ${pi + 1}`;
        const page = doc.loadPage(pi), pdfBoxes = structuredBoxes(page);
        let rec;
        try { rec = await recognizeRightStrip(page); } catch (e) { console.warn('safe primary titleblock OCR', item.name, pi + 1, e); continue; }
        for (const c of pending) {
          if (Number(c.ocrCount || 0) > 0) continue;
          const rawMatches = findWords(rec.data, c.find, rec.ox);
          const matches = rawMatches.filter(m => !overlapsPdfText(pdfBoxes, m.bbox));
          for (const m of rawMatches) if (!matches.includes(m)) diag('ocr.titleblock.primary.reject.pdftext', { file: item.name, page: pi + 1, target: c.find, ocrText: m.ocrText, bbox: m.bbox });
          if (!matches.length) continue;
          c.ocrCount = Number(c.ocrCount || 0) + matches.length;
          c.ocrMatches = (c.ocrMatches || []).concat(matches.map(m => ({ ...m, page: pi + 1 })));
          c.pages = c.pages || []; if (!c.pages.includes(pi + 1)) c.pages.push(pi + 1);
          total += matches.length;
        }
      }
    } finally { doc.destroy(); }
  }
  updateUI(batch, total);
  window.__titleBlockOCR = { total, version: 'safe-primary-v1' };
}
function waitForAnalysis(token) {
  let ticks = 0;
  const timer = setInterval(() => {
    if (token !== runToken) { clearInterval(timer); return; }
    const btn = document.querySelector(ANALYZE);
    if (btn && !btn.disabled && Array.isArray(window.__batchAnalysis) && window.__batchAnalysis.length) {
      clearInterval(timer); supplement(token).catch(e => console.warn('safe primary titleblock OCR supplement', e)); return;
    }
    if (++ticks > 2400) clearInterval(timer);
  }, 250);
}
document.querySelector(ANALYZE)?.addEventListener('click', () => {
  if (document.querySelector(OCR)?.checked !== true) return;
  runToken++; waitForAnalysis(runToken);
});
window.__safePrimaryTitleblockOCRLoaded = { version: 1 };
