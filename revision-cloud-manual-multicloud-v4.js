// Manual-only multi-cloud fallback v4. It never changes the proven automatic detector/remover.
// v4 keeps every v3 safety gate, but allows a denser SMALL component (<=40 strokes) up to 0.45.
// Larger components keep the original 0.22 density ceiling. It is reached only after normal and single-family manual removal fail.
const EPS=1e-6;
...
function diag(stage,extra={}){ try{ window.__cloudDiagnostic?.({stage,detail:'manual-cloud-multicloud-v4',...extra}); }catch(_){} }
...
function transformRect(rect,m){
  if(!rect||rect.length<4||!m||m.length<6)return rect?.slice?.()||rect;
  const pts=[[rect[0],rect[1]],[rect[2],rect[1]],[rect[0],rect[3]],[rect[2],rect[3]]].map(([x,y])=>[x*m[0]+y*m[2]+m[4],x*m[1]+y*m[3]+m[5]]);
  return [Math.min(...pts.map(p=>p[0])),Math.min(...pts.map(p=>p[1])),Math.max(...pts.map(p=>p[0])),Math.max(...pts.map(p=>p[1]))];
}
function intersectionArea(a,b){ if(!a||!b)return 0; return Math.max(0,Math.min(a[2],b[2])-Math.max(a[0],b[0]))*Math.max(0,Math.min(a[3],b[3])-Math.max(a[1],b[1])); }
function visibleScore(boxes,pageBounds){ let inside=0,total=0; for(const b of boxes||[]){ const a=area(b); if(a<=0)continue; total+=a; inside+=intersectionArea(b,pageBounds); } return total>0?inside/total:0; }
function alignBoxesForRaster(mupdf,page,boxes,pageBounds,rasterBBox){
  const raw=(boxes||[]).map(b=>b.slice()),rawScore=visibleScore(raw,pageBounds),rawHit=raw.some(b=>rectIntersects(b,rasterBBox));
  let converted=null,convertedScore=-1,convertedHit=false;
  try{
    const pageToPdf=Array.from(page.getTransform?.()||[]);
    if(pageToPdf.length>=6){
      const pdfToPage=mupdf.Matrix.invert(pageToPdf);
      converted=raw.map(b=>transformRect(b,pdfToPage));
      convertedScore=visibleScore(converted,pageBounds);
      convertedHit=converted.some(b=>rectIntersects(b,rasterBBox));
    }
  }catch(_){}
  const useConverted=!!converted&&(convertedScore>rawScore+0.25||(!rawHit&&convertedHit&&convertedScore>=0.98&&rawScore<0.98));
  return {boxes:useConverted?converted:raw,mode:useConverted?'pdf-user-to-page':'page',score:useConverted?convertedScore:rawScore,hit:useConverted?convertedHit:rawHit,rawScore,convertedScore,rawHit,convertedHit};
}

function evaluateMultiFamily(mupdf,page,key,strokes,pageBounds,rasterBBox,trace={}){
  trace.strokes=strokes.length;
  if(strokes.length<40||strokes.length>1200){ trace.reason=`tamaño fuera de rango ${strokes.length}`; return null; }
  ...
  trace.componentSizes=comps.map(c=>c.length);
  trace.gapLimit=gapLimit;
  if(good.length<2||good.length>8){ trace.reason=`componentes válidos=${good.length} · tamaños=${trace.componentSizes.join('+')}`; return null; }
  const goodCount=good.reduce((n,c)=>n+c.length,0);
  if(goodCount!==strokes.length){ trace.reason=`trazos sueltos=${strokes.length-goodCount} · componentes=${trace.componentSizes.join('+')}`; return null; }
  const pageArea=Math.max(1,area(pageBounds)),componentBoxes=[],componentSizes=[],componentDensity=[];
  let componentArea=0;
  for(let i=0;i<good.length;i++){
    const comp=good[i],box=familyUnion(comp),w=Math.max(1,box[2]-box[0]),h=Math.max(1,box[3]-box[1]),aspect=Math.min(w,h)/Math.max(w,h);
    if(w<40||h<40||aspect<0.10){ trace.reason=`geometría componente ${i+1}: ${w.toFixed(2)}x${h.toFixed(2)} · aspect=${aspect.toFixed(4)}`; return null; }
    const density=rectUnionArea(comp.map(s=>s.bbox))/Math.max(1,area(box));
    componentDensity.push(density);
    const densityLimit=comp.length<=40?0.45:0.22;
    if(density>densityLimit){ trace.reason=`densidad componente ${i+1}=${density.toFixed(5)} > ${densityLimit.toFixed(5)} · trazos=${comp.length} · tamaños=${trace.componentSizes.join('+')}`; return null; }
    componentArea+=area(box); componentBoxes.push(box); componentSizes.push(comp.length);
  }
  const areaFraction=componentArea/pageArea;
  trace.componentDensity=componentDensity.slice();
  trace.componentAreaFraction=areaFraction;
  if(areaFraction>0.08){ trace.reason=`área componentes=${areaFraction.toFixed(5)} > 0.08000`; return null; }
  const aligned=alignBoxesForRaster(mupdf,page,componentBoxes,pageBounds,rasterBBox);
  trace.aligned=aligned;
  if(!aligned.hit){ trace.reason=`sin intersección raster · rawHit=${aligned.rawHit} convertedHit=${aligned.convertedHit} · rawScore=${Number(aligned.rawScore).toFixed(5)} convertedScore=${Number(aligned.convertedScore).toFixed(5)}`; return null; }
  let bbox=null; for(const box of componentBoxes)bbox=unionRect(bbox,box);
  trace.reason=`aceptada · densidades=${componentDensity.map(x=>x.toFixed(5)).join('+')} · área=${areaFraction.toFixed(5)} · coord=${aligned.mode}`;
  return {key,rgb:strokes[0].rgb,lineWidth:strokes[0].lineWidth,strokeCount:strokes.length,componentCount:good.length,componentBoxes,componentSizes,componentDensity,bbox,gapLimit,componentAreaFraction:areaFraction,rasterCoordinateMode:aligned.mode,rasterComponentBoxes:aligned.boxes};
}
...
export async function removeManualMultiCloudBlock(data,detectedPages,options={}){
  ...
  try{
    ...
    for(const [key,strokes] of groups){ const trace={}; const c=evaluateMultiFamily(mupdf,page,key,strokes,pageBounds,rasterBBox,trace); if(strokes.length>=40)diag(c?'cloud.manual.multi.family.accept':'cloud.manual.multi.family.reject',{file,page:pageNo,strokes:strokes.length,rgb:strokes[0]?.rgb,lineWidth:strokes[0]?.lineWidth,reason:trace.reason||'rechazo sin detalle'}); if(c)candidates.push(c); }
    ...
    diag('cloud.manual.multi.accept',{file,page:pageNo,components:candidate.componentCount,componentSizes:candidate.componentSizes,strokes:candidate.strokeCount,rgb:candidate.rgb,lineWidth:candidate.lineWidth,bbox:candidate.bbox,coordinateMode:candidate.rasterCoordinateMode});
