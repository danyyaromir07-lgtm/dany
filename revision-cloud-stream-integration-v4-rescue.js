...
// v4 rescue: when "Eliminar nubes" is enabled, always give the exact/additive
// remover a chance to inspect the PDF. New additive families may intentionally
// be absent from the legacy analysis counter, so an empty revisionClouds array
// must not prevent Preview/Apply rescue.
...
async function applyExactFamilyCloudRemoval(){
  ...
  for(const item of batch){
    ...
    try{
      ...
      // Zero is not an error here: the additive inspection is deliberately allowed
      // even when legacy analysis found no cloud. Only explicit removal diagnostics
      // from a proven/detected candidate should be treated as failures.
      else if(detected.length&&Array.isArray(result?.details)&&result.details.length){
  ...
  window.__revisionCloudStreamApplyDebug={removed,failures,version:'v4-rescue-zero-analysis'};
...
function install(){
  ...
  wrapped.__exactCloudStreamVersion='v4-rescue-zero-analysis';
...
window.__revisionCloudStreamIntegration={version:'v4-rescue-zero-analysis'};
