...
function makeStructuralReplaceTransform(patterns){const maxPat=Math.max(...patterns.map(p=>p.findToken.length)),tailKeep=maxPat+AFTER_BYTES;let tail=new Uint8Array(0),history=new Uint8Array(0),rawHits=0,proven=0;
  ...
  function process(data,emitLimit,controller,flush=false){const hits=[];for(const p of patterns)for(const at of findAll(data,p.findToken,flush?data.length:emitLimit)){const end=at+p.findToken.length;if(!flush&&end+AFTER_BYTES>data.length)continue;rawHits++;const pre=contextFor(data,at),post=data.subarray(at,Math.min(data.length,end+AFTER_BYTES)),win=new Uint8Array(pre.length+post.length);win.set(pre);win.set(post,pre.length);const relStart=pre.length,relEnd=relStart+p.findToken.length;if(proveWindow(win,relStart,relEnd,p.fontNames)){hits.push({at,end,p});proven++}}
