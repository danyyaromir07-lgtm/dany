// Manual-only multi-cloud fallback v2. It never changes the proven automatic detector/remover.
// v2 only normalizes vector bboxes into MuPDF page space before comparing them with the raster candidate.
...
function diag(stage,extra={}){ try{ window.__cloudDiagnostic?.({stage,detail:'manual-cloud-multicloud-v2',...extra}); }catch(_){} }
...
function evaluateMultiFamily(mupdf,page,key,strokes,pageBounds,rasterBBox){
  ...
  if(componentArea/pageArea>0.08)return null;
  const aligned=alignBoxesForRaster(mupdf,page,componentBoxes,pageBounds,rasterBBox);
  if(!aligned.hit)return null;
  let bbox=null; for(const box of componentBoxes)bbox=unionRect(bbox,box);
  return {key,rgb:strokes[0].rgb,lineWidth:strokes[0].lineWidth,strokeCount:strokes.length,componentCount:good.length,componentBoxes,componentSizes,componentDensity,bbox,gapLimit,componentAreaFraction:componentArea/pageArea,rasterCoordinateMode:aligned.mode,rasterComponentBoxes:aligned.boxes};
}
...
export async function removeManualMultiCloudBlock(data,detectedPages,options={}){
  ...
  try{
    ...
    for(const [key,strokes] of groups){ const c=evaluateMultiFamily(mupdf,page,key,strokes,pageBounds,rasterBBox); if(c)candidates.push(c); }
