...
function pathOps(path, ctm) {
  ...
  path.walk({
    ...
    curveTo(x1, y1, x2, y2, x3, y3) {
      const a = transformPoint(x1, y1, ctm), b = transformPoint(x2, y2, ctm), c = transformPoint(x3, y3, ctm);
      ops.push(['c', a[0], a[1], b[0], b[1], c[0], c[1]]);
    },
...
function rasterComponents(page, scale = 0.35) {
  ...
  for (let i = 0, p = 0; i < red.length; i++, p += comps) {
    ...
    red[i] = (r >= 170 && g <= 125 && b <= 125 && r >= g + 70 && r >= b + 70) ? 1 : 0;
  }
  const seen = new Uint8Array(w * h);
  const stack = new Int32Array(w * h);
  const pageBounds = rectArray(page.getBounds());
  const out = [];
  for (let sy = 0; sy < h; sy++) {
    for (let sx = 0; sx < w; sx++) {
      const seed = sy * w + sx;
      if (!red[seed] || seen[seed]) continue;
      ...
      for (let y = minY; y <= maxY; y++) {
        ...
        for (let x = minX; x <= maxX; x++) {
          if (seen[y * w + x] && red[y * w + x]) { lo = Math.min(lo, x); hi = Math.max(hi, x); }
        }
        if (hi >= 0) {
      ...
      const scaleX = (pageBounds[2] - pageBounds[0]) / Math.max(1, w);
      const scaleY = (pageBounds[3] - pageBounds[1]) / Math.max(1, h);
      out.push({
        box: [pageBounds[0] + minX * scaleX, pageBounds[1] + minY * scaleY, pageBounds[0] + (maxX + 1) * scaleX, pageBounds[1] + (maxY + 1) * scaleY],
        area,
        density,
        roughness,
      });
...
function collectRedPaths(page, cloudBoxes) {
  const paths = [];
  if (!cloudBoxes.length) return paths;
  const device = new mupdf.Device({
    strokePath(path, stroke, ctm, _cs, color, alpha) {
      if (!rgbRed(color) || Number(alpha ?? 1) < 0.35) return;
      const width = Number(stroke?.lineWidth ?? 0);
      if (width <= 0 || width > 1.5) return;
      let bounds;
      try { bounds = rectArray(path.getBounds(stroke, ctm)); } catch (_) { return; }
      if (!bounds || rectArea(bounds) <= 0) return;
      const hit = cloudBoxes.some(c => rectIntersects(bounds, c.box, Math.max(1.5, width * 2.5)));
      if (!hit) return;
...
function serializeMask(paths) {
  ...
  for (const p of paths) {
    const width = Math.max(0.32, Math.min(1.6, p.width * 2.2));
    out.push(`${n(width)} w`);
...
export async function detectRevisionClouds(data) {
  ...
  try {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const boxes = rasterComponents(page);
      if (!boxes.length) continue;
      const paths = collectRedPaths(page, boxes);
      if (!paths.length) continue;
      const matched = [];
      for (const box of boxes) {
        const inside = paths.filter(p => rectIntersects(p.bounds, box.box, 1.5));
        if (inside.length < 20) continue;
        matched.push({ box: box.box, pathCount: inside.length, density: box.density, roughness: box.roughness });
      }
...
export async function removeRevisionClouds(data, detected) {
  ...
  try {
    for (const pageInfo of detected) {
      ...
      const paths = collectRedPaths(page, pageInfo.clouds);
      if (!paths.length) continue;
      appendContent(doc, page, serializeMask(paths));
      count += pageInfo.clouds.length;
    }
    const buffer = doc.saveToBuffer('garbage=4,compress=yes,appearance=yes');
    return { bytes: new Uint8Array(buffer.asUint8Array()), count };
  } finally { doc.destroy(); }
}
...
async function analyzeVisibleBatch() {
  ...
  for (const item of batch) {
    ...
    try {
      ...
      item.revisionCloudCount = detected.reduce((sum, p) => sum + p.clouds.length, 0);
    } catch (e) {
      item.revisionClouds = [];
  ...
  if (total) {
    const s = q(SUMMARY);
    if (s) {
      s.textContent = (s.textContent || '').replace(/ · ☁️[^·]*/g, '') + ` · ☁️ ${total} nube${total === 1 ? '' : 's'} de revisión detectada${total === 1 ? '' : 's'}`;
      s.classList.remove('hidden');
    }
  }
  q(STATUS) && (q(STATUS).textContent = total ? `Detectadas ${total} nube${total === 1 ? '' : 's'} de revisión gráfica${total === 1 ? '' : 's'}.` : 'No se detectaron nubes de revisión gráficas.');
}
...
function wire() {
  ...
  q('#batchAnalyze')?.addEventListener('click', () => {
    setTimeout(() => analyzeVisibleBatch().catch(e => console.error('[revision-clouds]', e)), 900);
  });
}
