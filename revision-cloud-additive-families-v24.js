// Additive q/cm cloud fallback v24: support CAD fragments whose scale+translation
// are combined in one cm matrix: q a 0 0 a tx ty cm 0 0 m ... S Q.
// v23/v22 remain unchanged and run first. This is an additive structural family.
import { detectAdditiveRevisionCloudFamilies as detectV23, removeAdditiveRevisionCloudFamilies as removeV23 } from './revision-cloud-additive-families-v23.js?v=20260831-tokendiag1';

const SRC='vector-red-exploded-qcm-combined-v24';
...
function parseCombinedFragment(bytes,i){
  let t=expect(bytes,i,'q');if(!t)return null;i=t.e;
  const m=matrix(bytes,i);if(!m)return null;i=m.end;
  const[a,b,c,d,tx,ty]=m.vals;
  if(!same(a,d)||!same(b,0)||!same(c,0)||!(a>0&&a<10)||!Number.isFinite(tx)||!Number.isFinite(ty))return null;
  const x0=tok(bytes,i),y0=x0&&tok(bytes,x0.e),mop=y0&&expect(bytes,y0.e,'m');
  if(num(x0)!==0||num(y0)!==0||!mop)return null;i=mop.e;
  let bbox=add(null,tx,ty),cur=[tx,ty],curves=0,lines=0,nums=[];
  ...
  while(true){
    ...
    if(q.t==='c'){
      if(nums.length<6)return null;const v=nums.slice(-6);for(let k=0;k<6;k+=2)point(v[k],v[k+1]);cur=[tx+v[4]*a,ty+v[5]*a];curves++;nums=[];continue;
    }
    if(q.t==='v'||q.t==='y'){
      if(nums.length<4)return null;const v=nums.slice(-4);for(let k=0;k<4;k+=2)point(v[k],v[k+1]);cur=[tx+v[2]*a,ty+v[3]*a];curves++;nums=[];continue;
    }
    if(q.t==='l'){
  ...
  const Q=expect(bytes,i,'Q');if(!Q)return null;
  return{end:Q.e,scale:a,startPoint:[tx,ty],endPoint:cur,bbox,curves,lines};
}
...
function readCombinedRun(b,start,end){
  let pos=end,bbox=null,paths=0,curves=0,lines=0,connected=0,prev=null,diags=[];
  while(pos<b.length){
    const f=parseCombinedFragment(b,pos);if(!f)break;
  ...
  return{start,end:pos,pathCount:paths,curveCount:curves,lineCount:lines,bbox,connectedFraction:conn};
}

function scanObjects(doc){
  const out=[],stats={objects:0,streams:0,readable:0,errors:0,redOps:0,runs:0};
  ...
  for(let xref=1;xref<stats.objects;xref++){
    try{
      ...
      stats.streams++;const bytes=new Uint8Array(ref.readStream().asUint8Array());stats.readable++;
      const reds=redOffsets(bytes);stats.redOps+=reds.length;
      for(const r of reds){const c=readCombinedRun(bytes,r.start,r.end);if(c){stats.runs++;out.push({...c,xref,stream:ref,bytes})}}
...
function diag(stage,extra={}){try{window.__cloudDiagnostic?.({stage,detail:'additive-qcm-v24-combined-matrix',...extra})}catch(_){}}
...
async function detectNew(data,context={}){
  ...
  try{
    ...
    const summary=out.map(c=>`x${c.xref}:p${c.pathCount}/c${c.curveCount}/l${c.lineCount}/conn${c.connectedFraction.toFixed(3)}`).join(', ');
    diag('cloud.qcm24.inspect',{file:context.file,candidates:out.length,groups:out.length,reason:`objetos=${stats.objects} streams=${stats.streams} legibles=${stats.readable} errores=${stats.errors} RGrojos=${stats.redOps}${summary?` | ${summary}`:''}`});
...
async function removeNew(data,pages){
  ...
  const mupdf=await import('https://cdn.jsdelivr.net/npm/mupdf@1.28.0/dist/mupdf.js'),doc=mupdf.PDFDocument.openDocument(new Uint8Array(data),'application/pdf');
  try{
    const {out}=scanObjects(doc),w=wanted[0],ms=out.filter(c=>c.xref===Number(w.vectorObjectXref)&&c.pathCount===Number(w.vectorAdditiveFamilyPathCount)&&c.curveCount===Number(w.vectorAdditiveFamilyCurveCount)&&c.lineCount===Number(w.vectorAdditiveFamilyLineCount)&&close(c.bbox,w.bbox));
    ...
    try{const remain=scanObjects(check).out.some(x=>x.pathCount===c.pathCount&&x.curveCount===c.curveCount&&x.lineCount===c.lineCount&&close(x.bbox,c.bbox));if(remain)return{data:new Uint8Array(data),removed:0,details:[{removed:false,reason:`${SRC}: verificación posterior falló`}]}}finally{check.destroy()}
    return{data:outBytes,removed:1,details:[{removed:true,source:SRC,xref:c.xref,paths:c.pathCount,curves:c.curveCount,lines:c.lineCount}]};
  }catch(err){return{data:new Uint8Array(data),removed:0,details:[{removed:false,reason:err?.message||String(err)}]}}
  finally{doc.destroy()}
}

export async function detectAdditiveRevisionCloudFamilies(data,context={}){return merge(await detectV23(data,context),await detectNew(data,context))}
export async function removeAdditiveRevisionCloudFamilies(data,pages,options={}){const first=await removeNew(data,pages);const second=await removeV23(first.data,pages,options);return{data:second.data,removed:Number(first.removed||0)+Number(second.removed||0),details:[...(first.details||[]),...(second.details||[])]}}
if(typeof window!=='undefined')window.__revisionCloudAdditiveV24={version:'24-combined-matrix1'};
