...
function partialWordMatch(word,target,originX,originY){
  ...
  if(!wanted||idx.key.length<wanted.length||!word?.bbox)return null;
  const start=idx.key.indexOf(wanted);
  if(start<0||confidence<10)return null;
  const startRaw=idx.positions[start],endPos=idx.positions[Math.min(idx.positions.length-1,start+wanted.length-1)];
  ...
  return{bbox:[originX+x0/SCALE,originY+b.y0/SCALE,originX+x1/SCALE,originY+b.y1/SCALE],confidence,similarity:1,ocrText:word.text,exact:true,titleBlockFallback:true,focusedTitleBlockRetry:true,partialWithinOCR:idx.key!==wanted,containerText:word.text,partialStart:startRaw,partialEnd:endRaw};
}
...
function findWords(data,target,originX,originY){
  ...
  for(let i=0;i<words.length;i++){
    ...
    for(let j=i;j<Math.min(words.length,i+12);j++){
      ...
      const k=key(text);
      if(k===wanted&&confidence>=10){found.push({bbox:[originX+box[0]/SCALE,originY+box[1]/SCALE,originX+box[2]/SCALE,originY+box[3]/SCALE],confidence,similarity:1,ocrText:text,exact:true,titleBlockFallback:true,focusedTitleBlockRetry:true});break}
...
function refreshTotals(batch){
  ...
  const summary=document.querySelector('#batchSummary');if(summary)summary.textContent=(summary.textContent||'').replace(/\d+ coincidencia(?:s)? vectorial\/OCR/,`${ocrTotal} coincidencia${ocrTotal===1?'':'s'} vectorial/OCR`);
  const apply=document.querySelector('#batchApply');if(apply)apply.disabled=!batch.some(a=>!a?.error&&((a.counts||[]).some(c=>c.count||c.annotationCount||c.ocrCount)||Number(a.comments||0)>0));
}
  ...
  for(let ai=0;ai<batch.length;ai++){
    ...
    try{
      for(let pi=0;pi<doc.countPages()&&pending.some(c=>Number(c.ocrCount||0)===0);pi++){
        if(token!==runToken)return;const page=doc.loadPage(pi),status=document.querySelector(STATUS);
        for(const c of pending){if(Number(c.ocrCount||0)>0)continue;if(status)status.textContent=`OCR cartela focalizado · ${item.name} · página ${pi+1}`;let matches=[];try{matches=await findOnPage(page,c.find)}catch(e){diag('ocr.titleblock.focused.error',{file:item.name,page:pi+1,target:c.find,error:e?.message||String(e)});console.warn('focused titleblock OCR v2',item.name,pi+1,e)}if(!matches.length)continue;c.ocrCount=matches.length;c.ocrMatches=matches.map(m=>({...m,page:pi+1}));c.pages=c.pages||[];if(!c.pages.includes(pi+1))c.pages.push(pi+1);total+=matches.length;diag('ocr.titleblock.focused.found',{file:item.name,page:pi+1,target:c.find,count:matches.length,ocrText:matches[0]?.ocrText,confidence:matches[0]?.confidence});refreshRow(ai)}
      }
    }finally{doc.destroy()}
  }
  refreshTotals(batch);const status=document.querySelector(STATUS);
  if(total){if(status)status.textContent=`Reconocimiento terminado: ${total} coincidencia${total===1?'':'s'} exacta${total===1?'':'s'} adicional${total===1?'':'es'} detectada${total===1?'':'s'} en cartela.`}
  else if(status&&/^OCR cartela(?: focalizado)? ·/.test(status.textContent||''))status.textContent='Análisis completado. Sin coincidencias adicionales exactas en OCR focalizado de cartela.';
  window.__focusedTitleBlockOCR={total,pruned,version:2};
}

function waitForPrimary(token,previousMarker){let ticks=0;const timer=setInterval(()=>{if(token!==runToken){clearInterval(timer);return}const marker=window.__titleBlockOCR;if(marker&&marker!==previousMarker){clearInterval(timer);setTimeout(()=>supplement(token).catch(e=>console.warn('focused titleblock OCR v2',e)),100);return}if(++ticks>3000)clearInterval(timer)},200)}

document.querySelector(ANALYZE)?.addEventListener('click',()=>{if(document.querySelector(OCR)?.checked!==true)return;const previousMarker=window.__titleBlockOCR;runToken++;diag('ocr.titleblock.focused.wait',{version:2});waitForPrimary(runToken,previousMarker)});
window.__focusedTitleBlockOCRLoaded={version:2};
