// Manual revision-cloud fallback. The proven automatic remover always runs first and is never relaxed.
...
function diag(stage, extra={}){ try{ window.__cloudDiagnostic?.({stage,detail:'manual-cloud-force-v1',...extra}); }catch(_){} }
...
async function tryManualVectorFamily(data, detectedPages, options, normal){
  ...
  if(options.context==='preview'&&options.file){ approvals.add(options.file); syncApprovals(); }
  return result;
}
...
export async function removeDetectedRevisionCloudsByExactFamily(data, detectedPages, options={}){
  ...
  try{
    return await tryManualVectorFamily(data,detectedPages,{context,file},normal);
  }catch(err){
    diag('cloud.manual.vector.error',{file,error:err?.message||String(err)});
    return manualFailure(normal,`modo manual: ${err?.message||String(err)}`);
  }
}
...
window.__revisionCloudManualForce={version:1,get approvedFiles(){return Array.from(approvals);}};
