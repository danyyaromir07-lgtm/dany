...
function flags(fn){if(typeof fn!=='function')return {prepare:'none'};return {prepare:'function',cloudSafe:!!fn.__cloudSafeWrap,exact:!!fn.__exactCloudStreamWrap,tolerance:!!fn.__cloudFailureToleranceWrap,oldProbe:!!fn.__preflightBreadcrumbWrap};}
let lastFn=null;
function observePrepareIdentity(){const fn=window.__prepareBatchAnnotationOperations;if(fn===lastFn)return;lastFn=fn;push('prepare function changed',flags(fn));}
...
function install(){push('trace v2 loaded');installClickTrace();observePrepareIdentity();installPerfObserver();let ticks=0;const timer=setInterval(()=>{observePrepareIdentity();installPerfObserver();if(++ticks>1200)clearInterval(timer);},25);}
