...
async function applyTextRule(doc, rule, expected, fileName, diagnostics) {
  let applied = 0;
  if (expected > 0) {
    try { applied += Number(editDoc(doc, rule.find, rule.replace) || 0); }
    catch (error) { diagnostics.push(`${fileName}: edición directa de «${rule.find}» falló; probando búsqueda PDF segura (${error?.message || String(error)})`); }
  }
  if (applied < expected) {
    ...
    } catch (error) { diagnostics.push(`${fileName}: fallback de texto PDF falló para «${rule.find}»: ${error?.message || String(error)}`); }
  }
  if (applied < expected) diagnostics.push(`${fileName}: «${rule.find}» encontrado=${expected}, aplicado=${applied}`);
  return applied;
}
...
export async function runFallback() {
  try {
    ...
    const vectorModule = await import('./vector-apply-safe-wrapper-v1.js?v=20260817-safecode1');
    const applyVectorOCR = vectorModule.applyVectorOCR;
    if (typeof applyVectorOCR !== 'function') throw new Error('No se pudo cargar el motor vector/OCR seguro.');
    const outputs = [], diagnostics = [];
    let totalEdits = 0, totalVector = 0, failures = 0;
    for (let i = 0; i < list.length; i++) {
      ...
      let work = new Uint8Array(source), doc = null, textEdits = 0, vectorEdits = 0;
      try {
        ...
        for (const rule of rules) {
          ...
          if (expected > 0) textEdits += await applyTextRule(doc, rule, expected, item.name, diagnostics);
          try {
            const freeTextResult = editFreeTextDetailed(doc, rule.find, rule.replace) || {};
            textEdits += Number(freeTextResult.count || 0);
          } catch (error) { diagnostics.push(`${item.name}: FreeText «${rule.find}» no pudo aplicarse: ${error?.message || String(error)}`); }
        }
        const vectorResult = applyVectorOCR(doc, item) || {};
        vectorEdits = Number(vectorResult.count || 0);
        if (Array.isArray(vectorResult.skipped) && vectorResult.skipped.length) diagnostics.push(`${item.name}: ${vectorResult.skipped.join(' · ')}`);
        if (textEdits || vectorEdits) work = savePdf(doc);
        outputs.push({ name: item.name, bytes: work });
        totalEdits += textEdits; totalVector += vectorEdits;
      } catch (error) {
...
window.__runBatchFallbackSafeV1 = runFallback;
