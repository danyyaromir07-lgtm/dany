...
old="return (v._ordinal??-1)+':d'+q.d.toFixed(2)+':l'+Number(v.lines||0)+':c'+Number(v.curves||0)+':s'+Number(v.segs||0)+':len'+L.toFixed(2)+':a'+A.toFixed(2)+':bb'+(v.bbox||[]).map(n=>Number(n).toFixed(1)).join('/')"
