...
import { editDoc } from './text-editor-v65.js?v=20260819-heavystream-base2';
...
async function applyTextRule(doc, rule, expected, fileName, diagnostics, prefix = '', heavyTextSafe = false) {
  let applied = 0;

  if (expected > 0 && !heavyTextSafe) {
    try {
  ...
  } else if (expected > 0 && heavyTextSafe) {
    diagnostics.push(`${fileName}: ${prefix}stream pesado · edición directa omitida; usando búsqueda PDF segura para «${rule.find}»`);
    perf('start', 'texto PDF · ruta segura búsqueda MuPDF', { file: fileName, find: rule.find, expected });
  }
  ...
  if (applied < expected) {
    ...
    try {
      ...
      if (heavyTextSafe) perf('end', 'texto PDF · ruta segura búsqueda MuPDF', { file: fileName, find: rule.find, expected, applied });
    }
  }
...
async function processItemOnce(item, source, applyVectorOCR, diagnostics, prefix = '') {
  ...
  const heavyTextSafe = Number(source?.byteLength || source?.length || 0) >= HEAVY_FILE_BYTES;
  ...
  try {
    ...
    if (heavyTextSafe && rules.some((r) => Math.max(0, Number(r.count || 0)) > 0)) {
      perf('start', 'texto PDF · archivo pesado protegido', { file: item.name, sizeBytes: Number(source?.byteLength || source?.length || 0) });
    }

    for (const rule of rules) {
      const expected = Math.max(0, Number(rule.count || 0));
      if (expected > 0) {
        pdfTextEdits += await applyTextRule(doc, rule, expected, item.name, diagnostics, prefix, heavyTextSafe);
      }
    ...
    if (heavyTextSafe && rules.some((r) => Math.max(0, Number(r.count || 0)) > 0)) {
      perf('end', 'texto PDF · archivo pesado protegido', { file: item.name, pdfTextEdits, freeTextEdits, vectorEdits });
    }
