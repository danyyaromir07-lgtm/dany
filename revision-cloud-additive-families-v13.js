// Additive revision-cloud families v13.
// Stable v11 families remain unchanged.
// v13 adds the strict c/v Bezier variant with newline-safe parsing.
import {
...
const SRC='vector-red-multiloop-bezier-cv-block-v13';
const NUM='[-+]?(?:\\d+(?:\\.\\d*)?|\\.\\d+)';
...
function readBlock(text,bodyStart){
  const tail=text.slice(bodyStart),parts=tail.split(/(\r?\n)/);let used=0,pathCount=0,curveCount=0,vCount=0,bbox=null,i=0;
  ...
  while(true){
    ...
    let curves=0,current=[+m[1],+m[2]];bbox=union(bbox,current);used+=raw.length;
    while(true){
      const r=nextLine();if(r==null)return null;const line=r.trim();let q=CURVE_C.exec(line);
      if(q){const p1=[+q[1],+q[2]],p2=[+q[3],+q[4]],p3=[+q[5],+q[6]];bbox=union(union(union(bbox,p1),p2),p3);current=p3;curves++;curveCount++;used+=r.length;continue}
      q=CURVE_V.exec(line);
      if(q){const p2=[+q[1],+q[2]],p3=[+q[3],+q[4]];bbox=union(union(bbox,p2),p3);current=p3;curves++;curveCount++;vCount++;used+=r.length;continue}
...
function pub(c){return{bbox:c.bbox,source:SRC,exactRGB:[1,0,0],vectorAdditiveFamilyProof:true,vectorAdditiveFamilyMode:'red-multiloop-bezier-cv-block-v13',vectorAdditiveFamilyPathCount:c.pathCount,vectorAdditiveFamilyCurveCount:c.curveCount,vectorAdditiveFamilyVCount:c.vCount}}
