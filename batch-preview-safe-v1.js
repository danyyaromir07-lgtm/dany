...
import { applyVectorOCR } from './vector-apply-safe-wrapper-v1.js?v=20260817-safecode1';
...
function applyTextAndVector(doc,a){let textEdits=0,vectorEdits=0;const warnings=[];for(const r of a.counts||[]){const expected=Math.max(0,Number(r.count||0));if(expected>0){let applied=0;try{applied+=Number(editDoc(doc,r.find,r.replace)||0)}catch(e){warnings.push(`Texto PDF directo «${r.find}»: ${e?.message||String(e)}`)}if(applied<expected){try{applied+=Number(editTextByPageSearch(doc,r.find,r.replace,expected-applied)||0)}catch(e){warnings.push(`Fallback texto PDF «${r.find}»: ${e?.message||String(e)}`)}}textEdits+=applied;if(applied<expected)warnings.push(`Texto PDF «${r.find}»: esperado=${expected}, preview=${applied}`)}const ft=editFreeTextDetailed(doc,r.find,r.replace)||{};textEdits+=Number(ft.count||0)}const result=applyVectorOCR(doc,a)||{};vectorEdits=Number(result.count||0);return{textEdits,vectorEdits,skipped:[...(result.skipped||[]),...warnings]}}
...
async function makePreview(idx){const batch=window.__batchAnalysis||[],a=batch[idx];if(!a||a.error)return;currentIndex=idx;currentPage=pageFor(a);window.__previewFileName=a.name;const msg=q('#batchStatus');if(msg)msg.textContent='Generando previsualización segura…';addUi();try{if(originalUrl)URL.revokeObjectURL(originalUrl);if(resultUrl)URL.revokeObjectURL(resultUrl);const source=new Uint8Array(a.data);const originalDoc=mupdf.PDFDocument.openDocument(new Uint8Array(source),'application/pdf');originalUrl=pixUrl(originalDoc,currentPage);originalDoc.destroy();
  ...
  if(msg)msg.textContent=applied.skipped.length?'Previsualización segura generada con avisos. Revisa el resultado antes de aplicar.':'Previsualización segura generada. El PDF original no se ha modificado.';
