...
function simpleWinAnsiFont(page,fontName,cache){if(cache.has(fontName))return cache.get(fontName);let ok=false;try{const po=page.getObject(),res=resolve(po.getInheritable('Resources')||po.get('Resources')),fonts=resolve(res?.get?.('Font')),ref=fonts?.isDictionary?.()?(fonts.get(fontName)||fonts.get('/'+fontName)):null;if(ref){const f=resolve(ref),subtype=String(f?.get?.('Subtype')||''),toUnicode=f?.get?.('ToUnicode'),enc=resolve(f?.get?.('Encoding'));let base='',hasDifferences=false;if(typeof enc?.asName==='function')base=String(enc.asName()||'');else if(enc?.isName?.())base=String(enc);else if(enc?.isDictionary?.()){const b=enc.get?.('BaseEncoding');base=typeof b?.asName==='function'?String(b.asName()||''):String(b||'');hasDifferences=!!enc.get?.('Differences')}ok=!!(/TrueType|Type1/.test(subtype)&&/WinAnsiEncoding/.test(base)&&!toUnicode&&!hasDifferences)}}catch(_){ok=false}cache.set(fontName,ok);return ok}
function collectSimpleSegments(bytes,page){const src=U(bytes),d=src instanceof Uint8Array?src:new Uint8Array(src),segs=[],fontCache=new Map();let i=0,inText=false,font=null,recent=[];const push=t=>{recent.push(t);if(recent.length>4)recent.shift()};while(i<d.length){while(i<d.length&&ws(d[i]))i++;if(i>=d.length)break;if(d[i]===37){while(i<d.length&&d[i]!==10&&d[i]!==13)i++;continue}const st=i,b=d[i];if(b===40){i=scanLiteral(d,i,d.length);if(inText)push({type:'string',kind:'literal',start:st,end:i});continue}if(b===60&&d[i+1]!==60){i=scanHex(d,i,d.length);if(inText)push({type:'string',kind:'hex',start:st,end:i});continue}if(b===91){const ar=scanArray(d,i,d.length);i=ar.end;if(inText)push(ar);continue}if(b===47){i++;while(i<d.length&&!del(d[i]))i++;if(inText)push({type:'name',start:st,end:i});continue}if(DEL.has(b)){i+=((b===60&&d[i+1]===60)||(b===62&&d[i+1]===62))?2:1;continue}i++;while(i<d.length&&!del(d[i]))i++;if(wordEq(d,st,i,'BT')){inText=true;recent=[];push({type:'word',start:st,end:i});continue}if(wordEq(d,st,i,'ET')){inText=false;recent=[];continue}if(!inText)continue;if(wordEq(d,st,i,'Tf')){const n=recent[recent.length-2];if(n?.type==='name')font=ascii(d.subarray(n.start+1,n.end))}else if(wordEq(d,st,i,'Tj')){const t=recent[recent.length-1];if(t?.type==='string'&&font&&simpleWinAnsiFont(page,font,fontCache))segs.push({tok:t,text:decodeSimpleToken(d,t)})}else if(wordEq(d,st,i,'TJ')){const ar=recent[recent.length-1];if(ar?.type==='array'&&font&&simpleWinAnsiFont(page,font,fontCache))for(const t of ar.items)segs.push({tok:t,text:decodeSimpleToken(d,t)})}push({type:'word',start:st,end:i})}return{d,segs}}
function editSimpleStream(bytes,needle,repl,page,maxHits){const{d,segs}=collectSimpleSegments(bytes,page);if(!segs.length)return{bytes:d,count:0};const full=segs.map(x=>x.text).join(''),matches=findMatches(full,needle).slice(0,maxHits);if(!matches.length)return{bytes:d,count:0};encodeWinAnsi(repl);const bases=[];let cur=0;for(const z of segs){bases.push(cur);cur+=z.text.length}const tokenEdits=new Map();for(const m of matches){const matchLen=Math.max(1,m.end-m.start);for(let k=0;k<segs.length;k++){const z=segs[k],base=bases[k],a=Math.max(0,m.start-base),b=Math.min(z.text.length,m.end-base);if(b<=a)continue;const rs=Math.round(((base+a-m.start)/matchLen)*repl.length),re=Math.round(((base+b-m.start)/matchLen)*repl.length);if(!tokenEdits.has(z.tok))tokenEdits.set(z.tok,{seg:z,edits:[]});tokenEdits.get(z.tok).edits.push({a,b,text:repl.slice(rs,re)})}}const changes=[];for(const{seg,edits}of tokenEdits.values()){let text=seg.text;edits.sort((x,y)=>y.a-x.a);for(const e of edits)text=text.slice(0,e.a)+e.text+text.slice(e.b);changes.push({start:seg.tok.start,end:seg.tok.end,replacement:encodeLiteral(text)})}changes.sort((a,b)=>a.start-b.start);const parts=[];let pos=0;for(const t of changes){parts.push(d.slice(pos,t.start),t.replacement);pos=t.end}parts.push(d.slice(pos));const n=parts.reduce((q,x)=>q+x.length,0),o=new Uint8Array(n);let p=0;for(const x of parts){o.set(x,p);p+=x.length}return{bytes:o,count:matches.length}}
