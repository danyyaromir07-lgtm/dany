...
async function worker(){if(workerPromise)return workerPromise;workerPromise=import('https://esm.sh/tesseract.js@5.1.0').then(({createWorker})=>createWorker('spa+eng')).then(async w=>{try{await w.setParameters({preserve_interword_spaces:'1',user_defined_dpi:'300',tessedit_pageseg_mode:'11'})}catch(_){}return w}).catch(e=>{workerPromise=null;throw e});return workerPromise}
async function recognize(canvas,mode){const w=await worker();await w.setParameters({preserve_interword_spaces:'1',user_defined_dpi:'300',tessedit_pageseg_mode:String(mode)});return (await w.recognize(canvas))?.data||null}
