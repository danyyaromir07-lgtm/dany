// Additive revision-cloud families v14.
// Stable v13 families remain unchanged.
// v14 adds one strict compact c-only Bezier cloud family.
// Safety: exact red stroke; isolated contiguous path block; every path is m + exactly 2 or 3 cubic c + S;
// compact range 40-79 paths prevents overlap with the stable >=80-path families.
// The RG color operator is deliberately preserved; only the proven Bezier path body is removed.
import {
  detectAdditiveRevisionCloudFamilies as detectV13,
  removeAdditiveRevisionCloudFamilies as removeV13
} from './revision-cloud-additive-families-v13.js?v=20260825-cvfix1';

const SRC='vector-red-compact-bezier-c-block-v14';
const NUM='[-+]?(?:\\d+(?:\\.\\d*)?|\\.\\d+)';
...
function readBlock(text,bodyStart){
  ...
  while(true){
    ...
    bbox=union(bbox,[+m[1],+m[2]]);used+=raw.length;let curves=0;
    while(true){
      const r=nextLine();if(r==null)return null;const line=r.trim();const q=CURVE_C.exec(line);
      if(q){const p1=[+q[1],+q[2]],p2=[+q[3],+q[4]],p3=[+q[5],+q[6]];bbox=union(union(union(bbox,p1),p2),p3);curves++;curveCount++;used+=r.length;continue}
      if(CURVE_V.test(line))return null;
      if(line!=='S')return null;used+=r.length;break;
    }
    if(curves<2||curves>3)return null;pathCount++;
    const peek=nextLine();if(peek==null)break;if(MOVE.test(peek.trim()))pending=peek;else{i-=2;break}
  }
  if(pathCount<40||pathCount>79||curveCount<pathCount*2||curveCount>pathCount*3||!bbox)return null;
  const w=bbox[2]-bbox[0],h=bbox[3]-bbox[1];if(Math.min(w,h)<300||Math.max(w,h)>20000)return null;
...
function pub(c){return{bbox:c.bbox,source:SRC,exactRGB:[1,0,0],vectorAdditiveFamilyProof:true,vectorAdditiveFamilyMode:'red-compact-bezier-c-block-v14',vectorAdditiveFamilyPathCount:c.pathCount,vectorAdditiveFamilyCurveCount:c.curveCount}}
...
async function removeNew(data,pages){const exp=wanted(pages);if(!exp.length)return{data:new Uint8Array(data),removed:0,details:[]};const mupdf=await import('https://cdn.jsdelivr.net/npm/mupdf@1.28.0/dist/mupdf.js'),doc=mupdf.PDFDocument.openDocument(new Uint8Array(data),'application/pdf'),details=[];let removed=0;try{for(const e of exp){if(e.page<1||e.page>doc.countPages())continue;const page=doc.loadPage(e.page-1),s=scanPage(page),ms=s.candidates.filter(c=>c.pathCount===Number(e.cloud.vectorAdditiveFamilyPathCount||0)&&c.curveCount===Number(e.cloud.vectorAdditiveFamilyCurveCount||0)&&closeBox(c.bbox,e.cloud.bbox,3));if(ms.length!==1){details.push({removed:false,page:e.page,reason:`${SRC}: revalidación=${ms.length}`});continue}const c=ms[0],st=s.streams[c.refIndex];st.ref.writeStream(toBytes(st.text.slice(0,c.start)+'\n'+st.text.slice(c.end)));if(scanPage(page).candidates.some(v=>v.pathCount===c.pathCount&&v.curveCount===c.curveCount&&closeBox(v.bbox,c.bbox,3)))throw new Error('compact Bezier cloud block persiste');removed++;details.push({removed:true,page:e.page,source:SRC,paths:c.pathCount,curves:c.curveCount})}if(!removed)return{data:new Uint8Array(data),removed:0,details};const buf=doc.saveToBuffer('garbage=4,compress=yes,appearance=yes');return{data:new Uint8Array(buf.asUint8Array()),removed,details}}catch(err){return{data:new Uint8Array(data),removed:0,details:[...details,{removed:false,reason:err?.message||String(err)}]}}finally{doc.destroy()}}
export async function detectAdditiveRevisionCloudFamilies(data,context={}){return merge(await detectV13(data,context),await detectNew(data))}
export async function removeAdditiveRevisionCloudFamilies(data,pages,options={}){const first=await removeNew(data,pages);const second=await removeV13(first.data,pages,options);return{data:second.data,removed:Number(first.removed||0)+Number(second.removed||0),details:[...(first.details||[]),...(second.details||[])]}}
