// Empty-aware text verification wrapper.
// Non-empty replacements preserve the existing v68 verification rule.
// Empty replacements are verified only by the exact decrease of the old text after save/reopen.
...
export function editDoc(doc,find,replace){
  ...
  const beforeNew=String(replace??'')===''?0:searchCount(doc,replace);
...
if(typeof window!=='undefined')window.__textEditorV68EmptyAware={version:'68-empty-aware-v1'};
