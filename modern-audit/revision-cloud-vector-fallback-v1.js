...
const VERSION='union-density-3+sweep-sync1';
...
function connectedComponents(strokes,gapLimit){
  ...
  for(const i of order){
    ...
    for(const j of active){const b=strokes[j].bbox;if(b[2]<minX)continue;next.push(j);const dy=Math.max(0,Math.max(r[1],b[1])-Math.min(r[3],b[3]));if(dy<=gapLimit&&rectGap(r,b)<=gapLimit)union(i,j);}
...
export async function detectVectorCloudFallback(data,context={}){
  ...
  try{
    for(let i=0;i<doc.countPages();i++){
      ...
      if(context.file){
        ...
        diag({stage:'cloud.perf.vector.page',detail:'vector-fallback-perf-v3-sync',file:context.file,page:i+1,reason:`${pageMs} ms · familias=${groups.size}`});
  ...
  if(context.file)diag({stage:'cloud.perf.vector.file',detail:'vector-fallback-perf-v3-sync',file:context.file,reason:`${Math.round((now()-t0)*10)/10} ms`});
  return out;
}
...
async function runFallbackAfterRaster(rasterBaseline){
  const started=now(),selected=q(CHECKBOX)?.checked===true;let batch=[],added=0,debug=[],status='not-selected',error=null;
  try{
    if(!selected)return;
    status='waiting-raster';
    for(let i=0;i<300;i++){
      batch=Array.isArray(window.__batchAnalysis)?window.__batchAnalysis:[];
      const baseReady=batch.length&&batch.every(x=>x?.error||x?.data);
      const rasterDone=window.__revisionCloudDebug!==rasterBaseline;
      if(baseReady&&rasterDone){status='running';break;}
      await sleep(100);
    }
    if(status!=='running'){status=batch.length?'raster-timeout':'no-batch';return;}
    ...
    diag({stage:'cloud.fallback.done',detail:'vector-fallback-perf-v3-sync',reason:`estado=${status} · ${elapsed} ms · añadidas=${added}`});
  }
}
function wire(){
  q('#batchAnalyze')?.addEventListener('click',()=>{
    const rasterBaseline=window.__revisionCloudDebug;
    diag({stage:'cloud.fallback.start',detail:'vector-fallback-perf-v3-sync',reason:'esperando final raster del ciclo actual'});
    runFallbackAfterRaster(rasterBaseline).catch(e=>console.error('[cloud-vector-fallback-v3]',e));
  });
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',wire);else wire();
window.__revisionCloudVectorFallbackV3={version:VERSION};
