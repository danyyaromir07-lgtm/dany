// Manual-only multi-cloud fallback v3 diagnostic. It never changes the proven automatic detector/remover.
// v3 keeps v2 acceptance rules exactly; it only records which safety gate rejects each family.
...
function diag(stage,extra={}){ try{ window.__cloudDiagnostic?.({stage,detail:'manual-cloud-multicloud-v3',...extra}); }catch(_){} }
...
function evaluateMultiFamily(mupdf,page,key,strokes,pageBounds,rasterBBox,trace={}){
  ...
  for(let i=0;i<good.length;i++){
    ...
    if(density>0.22){ trace.reason=`densidad componente ${i+1}=${density.toFixed(5)} > 0.22000 · tamaños=${trace.componentSizes.join('+')}`; return null; }
