...
function preservedFreeText(contents, rules) {
  const text = normalizedText(contents);
  return rules.some((rule) => { const needle = normalizedText(rule?.find); return needle && text.includes(needle); });
}
function removeDeferredHeavyComments(doc, item, rules, diagnostics) {
  if (!item?.__deferHeavyCommentsToApply) return 0;
  let removed = 0;
  crumb('comentarios diferidos · dentro de Apply pesado', { file: item.name || '' });
  perf('start', 'comentarios · dentro de Apply pesado', { file: item.name || '' });
  try {
    for (let i = 0; i < doc.countPages(); i++) {
      const page = doc.loadPage(i), annots = Array.from(page.getAnnotations?.() || []);
      for (const annot of annots) {
        ...
        if (type === 'FreeText') {
          ...
          if (preservedFreeText(contents, rules)) continue;
        }
        page.deleteAnnotation(annot);
        removed++;
      }
    }
    perf('end', 'comentarios · dentro de Apply pesado', { file: item.name || '', removed });
    crumb('comentarios diferidos · terminados', { file: item.name || '', removed });
    diagnostics.push(`${item.name}: comentarios/anotaciones diferidos eliminados dentro de Apply (${removed})`);
    return removed;
  } catch (error) {
    perf('end', 'comentarios · dentro de Apply pesado', { file: item.name || '', removed, warning: error?.message || String(error) });
    crumb('comentarios diferidos · error', { file: item.name || '', removed, error: error?.message || String(error) });
    throw error;
  }
}
...
async function processItemOnce(item, source, applyVectorOCR, diagnostics, prefix = '') {
  let doc = null, pdfTextEdits = 0, freeTextEdits = 0, vectorEdits = 0, commentsRemoved = 0, bytes = new Uint8Array(source);
  ...
  try {
    ...
    commentsRemoved = removeDeferredHeavyComments(doc, item, rules, diagnostics);
    ...
    if (commentsRemoved || pdfTextEdits || freeTextEdits || vectorEdits) {
      crumb('guardar PDF tras Apply', { file: item.name, commentsRemoved, pdfTextEdits, freeTextEdits, vectorEdits });
      bytes = savePdf(doc);
    }
    if (heavy && rules.some((r) => Math.max(0, Number(r.count || 0)) > 0)) perf('end', 'texto PDF · archivo pesado Flate protegido', { file: item.name, commentsRemoved, pdfTextEdits, freeTextEdits, vectorEdits });
    return { bytes, pdfTextEdits, freeTextEdits, vectorEdits, commentsRemoved, heavy };
  } finally {
...
export async function runFallback() {
  try {
    ...
    let totalEdits = 0, totalVector = 0, failures = 0, unresolvedFiles = 0, totalDeferredComments = 0;
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      try {
        ...
        totalDeferredComments += Number(result.commentsRemoved || 0);
    ...
    if (totalDeferredComments > 0 && $('#statComments')) $('#statComments').textContent = totalDeferredComments;
    ...
    if (summary) {
      ...
      summary.textContent = `${outputs.length} PDF${plural} procesado${plural} · ${totalEdits} edit${editPlural} de texto/FreeText · ${totalVector} edición${totalVector === 1 ? '' : 'es'} vector/OCR${totalDeferredComments ? ` · ${totalDeferredComments} comentario${totalDeferredComments === 1 ? '' : 's'}/anotación${totalDeferredComments === 1 ? '' : 'es'} eliminado${totalDeferredComments === 1 ? '' : 's'} en la pasada única` : ''}${unresolvedFiles ? ` · ⚠️ ${unresolvedFiles} archivo${unresolvedFiles === 1 ? '' : 's'} con sustituciones pendientes` : ' · ✓ todas las sustituciones PDF esperadas aplicadas'}${failures ? ` · ${failures} error${failures === 1 ? '' : 'es'}` : ''}${diagnostics.length ? ' · revisa el diagnóstico' : ''} · ZIP descargado`;
