...
function ensureLocationBox() {
  ...
  box = document.createElement('div');box.id = 'revisionCloudLocationBox';box.className = 'text-warning hidden';box.style.marginTop = '10px';status.insertAdjacentElement('afterend', box);return box;
}
function cloudRows() {
  const batch = Array.isArray(window.__batchAnalysis) ? window.__batchAnalysis : [];
  return batch.flatMap((item) => { if (!item || item.error) return []; const pages = Array.isArray(item.revisionClouds) ? item.revisionClouds.filter((p) => Array.isArray(p?.clouds) && p.clouds.length).map((p) => ({ page: Number(p.page), count: p.clouds.length })) : []; const count = pages.reduce((n, p) => n + p.count, 0); return count ? [{ name: item.name || '(sin nombre)', count, pages }] : []; });
}
function renderLocations() {
  if (!q(CHECKBOX)?.checked) return;const rows = cloudRows();const signature = JSON.stringify(rows);if (signature === lastSignature) return;lastSignature = signature;const box = ensureLocationBox();if (!box) return;
  if (!rows.length) { box.classList.add('hidden');box.textContent = '';return; }
  box.replaceChildren();const total = rows.reduce((n, r) => n + r.count, 0);const title = document.createElement('strong');title.textContent = `☁️ Ubicación de ${total} nube${total === 1 ? '' : 's'} de revisión`;box.appendChild(title);
  for (const row of rows) { const file = document.createElement('div');file.style.marginTop = '7px';file.textContent = `📄 ${row.name}`;box.appendChild(file);for (const p of row.pages) { const line = document.createElement('div');line.style.marginLeft = '18px';line.textContent = `↳ Página ${p.page}: ${p.count} nube${p.count === 1 ? '' : 's'}`;box.appendChild(line); } }
...
function loadDetailed() { import('./revision-cloud-diagnostics-v2.js?v=20260815-strokewidth1').catch((err) => { const summary = q('#cloudDiagSummary');const log = q('#cloudDiagLog');const msg = err?.message || String(err);if (summary) summary.textContent = 'No se pudo cargar el diagnóstico detallado.';if (log) log.textContent = `cloud.diagnostic.loader.error | ${msg}`; }); }
