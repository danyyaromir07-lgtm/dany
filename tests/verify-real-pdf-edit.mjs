...
function pageContents(page) {
  ...
  return refs.map(ref => {
    const st = resolve(ref);
    return st?.isStream?.() ? new Uint8Array(st.readStream()) : null;
  }).filter(Boolean);
}
function annotationCount(page) {
  const po = page.getObject();
  const ann = resolve(po.get('Annots'));
  return ann?.isArray?.() ? ann.length : 0;
}

const pageBefore = doc.loadPage(0);
const textBefore = pageText(pageBefore);
if (!textBefore.includes(expected.replace('O03', 'E03'))) throw new Error('fixture does not contain expected original text');
